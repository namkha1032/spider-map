...
const TreeNode = ({ node, root, isLastChild }) => {
    let antdTheme = theme.useToken()
    ...
    const items = [
        ...
        //     label: (
        //         <Typography.Text>
        //             {node.showDescription ? "Hide Description" : "Show Description"}
        //         </Typography.Text>
        //     ),
        //     icon: <EyeOutlined />,
        ...
        //     label: (
        //         <Typography.Text>
        //             {node.showChildren ? "Collapse" : "Expand"}
        //         </Typography.Text>
        //     ),
        //     icon: node.showChildren ? <DownOutlined /> : <RightOutlined />,
    ...
    return (
        <>
            {/* {root == false
                        ? <div style={{ width: cardWidth * 2 / 3, margin: "auto 0px" }}>
                            <HorizontalLine />
                        </div>
                        : <></>} */}

            <>
                <Flex vertical={true} style={{ marginTop: isLastChild ? 0 : marginValue }}>
                    <Flex>
                        <Flex vertical={true} style={{ minWidth: cardWidth / 2 }}>
                            <div onClick={toggleShowDescription} style={{ caretColor: "transparent", cursor: "pointer", minHeight: "50%", minWidth: cardWidth / 2, borderBottom: root ? "" : `1px solid ${lineColor}`, borderLeft: isLastChild ? `1px solid ${lineColor}` : "", borderRadius: isLastChild ? `0 0 0 ${radiusAmount}px` : "" }}>

                            </div>
                            <div onClick={toggleShowChildren} style={{ caretColor: "transparent", cursor: "pointer", minHeight: "50%", minWidth: cardWidth / 2 }}>
                            </div>
                        </Flex>
                        <Dropdown menu={{ items }} trigger={'contextMenu'}>
                            <div>
                                <NodeCard node={node} cardWidth={cardWidth} />
                            </div>
                        </Dropdown>
                    </Flex>
                    {
                        node?.children?.length > 0 && node?.showChildren ?
                            <>
                                <Flex>
                                    ...
                                    <div style={{ caretColor: "transparent", borderRight: `1px solid ${lineColor}`, minWidth: cardWidth / 2, minHeight: "100%" }}></div>
                                    <Flex vertical={true}>
                                        {node.children.map((child, index) => {
                                            if (index < node.children.length - 1) {
                                                return (
                                                    <TreeNode key={child.nodeID} node={child} root={false} isLastChild={false} />
                                                )
                                            }
                                            // else {
                                            //     return <></>
                                ...
                                <Flex vertical>
                                    ...
                                    <Flex id={node.nodeID.replace(" ", "_")}>
                                        ...
                                        <TreeNode node={node.children[node.children.length - 1]} root={false} isLastChild={true} />
                                    </Flex>
                                </Flex>
                            </>
                            : <>
        ...
        <>
            <CurrentTreeContext.Provider value={{ currentTree, setCurrentTree }}>
                <Content style={{ padding: '48px' }}>
                    ...
                    {currentTree ? <TreeNode node={currentTree} root={true} /> : <></>}
                </Content>
